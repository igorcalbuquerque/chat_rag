...
> localmente com `docker compose up`, a autenticação vem **desligada por padrão**, então você vai direto para a tela inicial, sem necessidade de login.
...
Esse arquivo guarda suas configurações e chaves. **Ele está no `.gitignore`**, então suas chaves nunca são commitadas.
...
8. Remova um documento com o **✕** na lista, e seus vetores são apagados do Redis.
...
fallback. Assim, um deploy público **não precisa de nenhuma chave embarcada**: cada visitante traz a sua.
...
- **Provedor configurável (OpenAI/Anthropic/Gemini/Ollama):** factories em
  ...
  100% local e grátis (Ollama) ou com APIs pagas/gratuitas trocando uma variável, e o visitante pode sobrescrever o LLM do chat por requisição (BYOK).
